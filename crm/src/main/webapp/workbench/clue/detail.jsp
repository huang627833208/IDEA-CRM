...
<script type="text/javascript">
	...
	$(function(){
		...
		//为搜索市场活动的文本框绑定事件，根据输入的关键字搜索市场活动列表
		//为此文本框绑定敲键盘事件，如果敲的是回车键，则执行搜索
		$("#aname").keydown(function (event) {

			//如果码值为13，说明敲的是回车键，执行搜索市场活动操作
			if(event.keyCode==13){

				//alert("发出ajax请求，根据传递的市场活动名称，搜索关联的市场活动列表");
				...
				//展现完市场活动列表后，应该及时终止掉该方法，否则模态窗口会执行默认触发的回车行为（强制刷新页面）
				return false;
		...
		$("#bundBtn").click(function () {
			...
			//选中了一条或者多条
				...
				/*
					...
					由于activityId有可能有多个，那么我们传递参数的形式有可能是同一个key下有多个值
					我们需要使用传统传递参数的形式
				...
				//以上参数处理完毕后，发出ajax请求做线索关联市场活动的操作
				$.ajax({
		...
		$.ajax({
			...
			success : function (data) {
				...
				$.each(data,function (i,n) {
					...
					//如果我们能够将n（市场活动）的id，处理成为关联关系表的id，解除关联操作就更容易了
