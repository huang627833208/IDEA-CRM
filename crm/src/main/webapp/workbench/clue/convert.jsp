...
<script type="text/javascript">
	$(function(){
		...
		//为搜索市场活动的文本框绑定敲键盘事件，如果敲的是回车键，需要查询并展现相应的市场活动列表
		...
		$("#submitActivityBtn").click(function () {
			...
			if($xz.length==0){
				...
				//取得选中的记录的id
				var id = $xz.val();
				//将选中记录的id填写到交易表单市场活动id的隐藏域中
				$("#activityId").val(id);

				//取得选中记录的name
				var name = $("#"+id).html();
				//将name填写到交易表单的市场活动名称中
		...
		$("#convertBtn").click(function () {
			...
			/*
				...
				发出传统请求，还是ajax请求？
				不需要在页面中执行任何局部刷新，所以发出传统请求即可


			 */

			//根据 "为客户创建交易" 的复选框 有没有选中，判断在做线索转换的同时要不要创建一笔交易

			/*

				关于为后台传递的参数
					不论是否创建交易，都需要为后台传递clueId
					如果需要创建交易，除了传递clueId之外，还得传递交易表单中的信息

			 */
			if($("#isCreateTransaction").prop("checked")){

				//需要创建交易
				//alert("执行线索转换操作，同时创建一笔交易");
				//window.location.href = "workbench/clue/convert.do?clueId=xxx&money=xxx&name=xxx&ex.....";
				//以上形式挂载参数比较麻烦，而且不利于参数的扩充
				//我们这次为了参数的有效传递，选择的技术是使用以提交表单的形式发出请求
				$("#tranForm").submit();
